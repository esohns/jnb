...
void
sendPacketToSock(TCPsocket s, struct NetPacket* pkt)
{
...
void
sendPacketToAll(struct NetPacket* pkt)
{
	int i;
...
void
processMovePacket(struct NetPacket* pkt)
{
...
void
tellServerPlayerMoved(int playerid, int movement_type, int newval)
{
	struct NetPacket pkt;
...
void
processKillPacket(struct NetPacket* pkt)
{
	int c1 = pkt->arg;
	int c2 = pkt->arg2;
	int x = pkt->arg3;
...
#ifdef USE_NET
void
processPositionPacket(struct NetPacket* pkt)
{
	int playerid = pkt->arg;
...
void
processAlivePacket(struct NetPacket* pkt)
{
	int playerid = pkt->arg;
